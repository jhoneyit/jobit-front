# jobit-front — JD 기반 기술 면접 준비 (프론트)

채용공고를 붙여넣으면 그 공고에 맞는 예상 면접 질문과 답변 뼈대를 만들어 주는 웹 서비스.
`jd-interview-prep-spec.md` 스펙의 **1단계** 구현 — JD 붙여넣기 → 파싱 → 질문 10개(스트리밍).

**2026-08-07: 면접 연습(음성)이 붙었다.** 분석한 공고의 질문에 소리 내어 답하면 답변 뼈대와
대조해 점수를 준다 — `/interview`, 기록은 `/profile/interviews`.
설계는 `jobit/docs/interview-practice-design.md`.

> **오디오는 이 레포를 지나가지 않는다.** STT 는 브라우저(Web Speech API)가 하고 백엔드로는
> 텍스트만 간다. `MediaRecorder` 를 쓰지 않는 이유가 이것이다 — 오디오를 만지지 않으면
> 실수로 보낼 경로도 생기지 않는다.
>
> 다만 **Chrome 은 인식을 위해 오디오를 구글 서버로 보낸다.** "우리 서버에 저장하지 않는다"는
> 사실이지만 그 이상을 말하면 거짓이라, 마이크를 켜기 전 화면에 그대로 밝힌다.
> Firefox 는 이 API 가 없어 텍스트 입력으로 떨어진다 (`useSpeechRecognition`).

## 두 레포 구성 (2026-08-03 확정)

이 제품은 레포 두 개다. 나란히 체크아웃해 두고 쓴다.

```
프로젝트/
  jobit/        Spring Boot. REST API + LLM + DB + 도메인 로직
  jobit-front/  ← 여기. UI · SSR · 세션
```
...
>
> **아직 남은 이관 대상**: `src/lib/admin/` 이 DB 를 직접 읽는다. (`src/lib/store.ts` 의
> 제출 이력은 2026-08-07 에 `/api/submissions` 로 옮겼다 — 남은 것은 공고·질문 조회뿐이다.)
>
...
- **스크롤 연출** — `motion` 으로 히어로 패럴랙스와 섹션 리빌(`Reveal`)을 만들고
  `lenis` 로 관성 스크롤을 겁니다.
...
- **`/` 랜딩** — 입력 폼은 없고 `/analyze` 로 보냅니다(위 §랜딩 페이지 참고).
  히어로 아래로 결과 예시 → 차별점 → 동작 방식 → 스트리밍 시연 → 이력서 갭 분석(3단계) 예고 순입니다.
...
- **`AUTH_URL` 을 운영에서 설정하세요.** `auth.ts` 에 `trustHost: true` 가 켜져 있습니다 —
  ...
  위조 가능성을 막는 편이 안전합니다.
- **가입 시점의 이메일 인증이 없습니다.** 재설정 링크를 열어야 인증됩니다 — 오타난 주소로
  가입하면 그 계정은 재설정으로 복구할 수 없습니다. (비밀번호 재설정 자체는 구현되어 있습니다.)
